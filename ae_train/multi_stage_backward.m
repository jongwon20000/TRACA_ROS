...
%         ww = bsxfun(@times, pagefun(@mtimes, permute(w{jj}, [2,1,4,3,5]), w{jj}), ...
%             permute(1-eye(size(w{jj},3), size(w{jj},3)), [3,4,1,2]));
        wi2 = pagefun(@mtimes, permute(w{jj}, [2,1,3,4,5]), w{jj}) + epsilon;
        wk2 = permute(wi2, [1,2,4,3,5]);
%         dEdw = sum(bsxfun(@times, ww, w{jj}),3);
...
%     dae_res{ii}{jj,1} = vl_simplenn(daenet{ii}(jj,1), n_mfeat, dae_res{ii}{jj,2}(1).dzdx, dae_res{ii}{jj,1}, 'SkipForward', true);
end
