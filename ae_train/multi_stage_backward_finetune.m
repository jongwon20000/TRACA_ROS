...
%estimate the correlation filters
...
%     dae_res{jj,2} = vl_simplenn(daenet(jj,2), dae_res{jj,1}(end).x, dEdz, dae_res{jj,2}, 'SkipForward', true);
    % orthogonality loss
    if(orth_lambda > 0.0)
        orth_der = dEdx{jj};
    else
...
%     dae_res{jj,1} = vl_simplenn(daenet(jj,1), n_mfeat, dae_res{jj,2}(1).dzdx + orth_lambda*orth_der, dae_res{jj,1}, 'SkipForward', true);
%     dae_res{jj,1} = vl_simplenn(daenet(jj,1), n_mfeat, orth_der, dae_res{jj,1}, 'SkipForward', true);
end
